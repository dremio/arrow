...
TEST(TestStringOps, TestSoundex) {
  ...
  // Basic test
  auto out = soundex_utf8(ctx_ptr, "Robert", 6, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(out_len, 4);
  EXPECT_EQ(std::string(out, out_len), "R163");

  // Ignores non-alphabetic
  out = soundex_utf8(ctx_ptr, "r-O-b-E-r-T", 11, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "R163");

  // More examples
  out = soundex_utf8(ctx_ptr, "Rupert", 6, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "R163");

  out = soundex_utf8(ctx_ptr, "Ashcraft", 8, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "A261");

  out = soundex_utf8(ctx_ptr, "Tymczak", 7, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "T522");

  out = soundex_utf8(ctx_ptr, "Pfister", 7, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "P236");

  // Short strings pad with zeros
  out = soundex_utf8(ctx_ptr, "A", 1, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "A000");

  out = soundex_utf8(ctx_ptr, "AB", 2, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(std::string(out, out_len), "A100");

  // No alphabetic returns empty (null result)
  out_valid = true;
  out = soundex_utf8(ctx_ptr, "123456789", 9, true, &out_valid, &out_len);
  EXPECT_FALSE(out_valid);

  // Null input
  out_valid = true;
  out = soundex_utf8(ctx_ptr, nullptr, 0, false, &out_valid, &out_len);
  EXPECT_FALSE(out_valid);

  // Empty string
  out_valid = true;
  out = soundex_utf8(ctx_ptr, "", 0, true, &out_valid, &out_len);
  EXPECT_TRUE(out_valid);
  EXPECT_EQ(out_len, 0);
}
