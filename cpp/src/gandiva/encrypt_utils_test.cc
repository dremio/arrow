...
TEST(TestShaEncryptUtils, TestAesEncryptDecrypt) {
  ...
  // check exception
  char* cipher = "JBB7oJAQuqhDCx01fvBRi8PcljW1+nbnOSMk+R0Sz7E==";
  ...
  ASSERT_THROW({
        gandiva::aes_encrypt(to_encrypt, to_encrypt_len, key, cipher_4);
    }, std::runtime_error);

  ASSERT_THROW({
        gandiva::aes_decrypt(cipher, cipher_len, key, plain_text);
    }, std::runtime_error);
  ...
  ASSERT_THROW({
        gandiva::aes_encrypt(to_encrypt, to_encrypt_len, key, cipher_5);
    }, std::runtime_error);
  ASSERT_THROW({
        gandiva::aes_decrypt(cipher, cipher_len, key, plain_text);
    }, std::runtime_error);  
