...
const char* gdv_fn_aes_encrypt_gcm(int64_t context, const char* data, int32_t data_len,
  ...
  try {
    ...
    if (iv_len < 0) {
      throw std::runtime_error(
          std::string("Invalid IV length for AES encryption: ") + std::to_string(iv_len) +
          " (must be >= 0)");
    }
...
const char* gdv_fn_aes_decrypt_gcm(int64_t context, const char* data, int32_t data_len,
  ...
  try {
    ...
    if (iv_len < 0) {
      throw std::runtime_error(
          std::string("Invalid IV length for AES decryption: ") + std::to_string(iv_len) +
          " (must be >= 0)");
    }
    ...
    if (cipher_len < 0) {
      throw std::runtime_error(
          ...
          " (must be <= data length " + std::to_string(data_len) + ")");
    }
